$(document).ready(function(){
  ...
  //loadFailureTable('../alcazar/runbook/get/all/fake', $("#userTbl"), map);
 ...
 /* $("#preBtn").click(function(){
  ...
  $("#userTbl tbody tr").live('click', function(event){	  
    ...
    for(var i = 0; i < jsonItem.conditions.length; i++){
    	appendFailureCondition(jsonItem.conditions[i], $("#conditionGroup"), i+1);
    }
  ...
  $("#addBtn").click(function (e) {
    ...
    var jsonItem = [];
    appendFailureCondition(jsonItem, $("#conditionGroup"), counter++);
  });
...
function appendFailureCondition(jsonItem, div, num){
  ...
  var condId = "cond" + num;
  var select = '<select id='+ condTypeId +' data-style="btn-warning"><option title="Combo 1">STRING_MATCH</option><option title="Combo 2">STRING_PATTERN</option><option title="Combo 3">STEP</option></select>';
  
  newTextBoxDiv.html('<label class="col-lg-2 control-label">Condition #'+ num + ' : </label><div class="col-lg-10">' + select +
